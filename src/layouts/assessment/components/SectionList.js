...
const SectionList = ({ assessment, section, setSection, handleSaveThread }) => {
    function ResumeField({ field }) {
        return (
            <MDTypography flexGrow={1} sx={{
                backgroundImage:
    ...
    return (
        <MDBox px={2} width="100%">
            <TransitionGroup>
                {section > 1 &&
                    <Collapse key={assessment.description} timeout={800}>
                        <MDBox display='flex' gap={1} m={0}>
                            <ResumeField field='name' />
                            <ResumeField field='type' />

                        </MDBox>

                        <ResumeField field='origines' />
                        <ResumeField field='description' />

                    </Collapse>}
                {section > 2 &&
                    <Collapse key={assessment.criticalImpact}>
                        <MDBox >
                            <ResumeField field='criticalImpact' />
                            <ResumeField field='ttp1' />
                            <ResumeField field='ttp2' />
                        </MDBox>
                    </Collapse>}
                {section > 3 &&
                    <Collapse key={assessment.id}>
                        <MDBox >
                ...
                {section > 4 &&
                    <Collapse key={assessment.threatComment}>
                        <MDBox >
